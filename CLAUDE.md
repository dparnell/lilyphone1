...
- **The LTR-553ALS does not answer on this board at all.** Probed at 0x23 cold and again with `BOARD_1V8_EN` driven high and 50ms to settle, it never acknowledges - so it is either not fitted on this revision or not on the rail the pin name suggests. Ear detect is written against it and works if one ever answers, but here it reports "No sensor" and does nothing. Before spending time on it again, read the boot I2C scan: it now names every address that answers, so whether 0x23 is silent while 0x28 (the other part on that rail) is not would settle whether the problem is the rail or the part.
