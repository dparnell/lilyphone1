...
- `BLE_MIN_INTERNAL_FREE` / `WIFI_MIN_INTERNAL_FREE` are checked before starting either radio, because the stacks abort on the way up rather than returning a failure. They are headroom guesses, not documented requirements; the free figure is logged and shown on the screen either way.
