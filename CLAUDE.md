...
One sensor is fitted, has a driver, and is read by nothing. A second was
removed.

- **There is no magnetometer on this board, so a compass is not possible.** Settled by asking the BHI260AP itself: it presents 33 virtual sensors and not one of them is a magnetometer (19, 21-25) or Orientation (43). The giveaway is `Game rotation vector` (37) - Bosch's six-axis fusion, the one used precisely when there is no magnetometer, whose yaw is relative to wherever it started and drifts. The only absolute heading available on this device is GPS course over ground, and only while moving.

  The BHI260AP is a hub rather than a fixed set of sensors, which is why this had to be asked at runtime: the accelerometer and gyroscope are its own, a magnetometer would be an external part on its secondary bus, and `0x28` answers on the main bus either way. The driver that asked did so through `SENSOR_ID_MAG_PASS` and `SENSOR_ID_ORI` and printed the hub's whole sensor list; it has since been removed, so a board that might have a magnetometer needs `src/peripherals/peri_gyroscope.cpp` recovered from git history to find out.

  The vendor firmware registered a magnetometer callback and never enabled the sensor, so `BHI260AP_get_val(3)` returned zeros forever and implied hardware that nobody had checked for. Do not take that kind of scaffolding as evidence.

  `SENSOR_ID_ORI` is the one worth having for a compass: the hub's own fusion of all three sensors, giving a heading already corrected for tilt and for the iron around it. Its Euler angles are in 1/32768ths of a turn.
- **The BHI260AP driver is gone, and so is the power to it.** Nothing read the hub, and its init uploaded a firmware image over I2C and then streamed accelerometer and gyroscope at 100Hz that nobody serviced. `src/peripherals/peri_gyroscope.cpp`, its declarations in `peripheral.h`, its `E_PERI_*` slot and the never-defined `ui_other_get_gyro()` were all removed; recover the file from git history to bring it back, along with a row in `boot_screen.cpp` and something to service `bhy.update()`. What the hub offers, if it is ever wanted: step counting (52), pickup and glance gestures (61, 59), significant motion (55), stationary detect (75) and activity recognition (63).

  **Deleting it saved 788 bytes of flash and 96 of RAM, not the ~115KB an earlier note in this file claimed.** The claim was about not *calling* the init, which was already true - the linker had long since dropped everything unreachable, so almost nothing was left to remove. The saving that does matter is current: `BOARD_1V8_EN` is now driven **low** at boot, where the vendor code drove it high, so a hub with no code behind it is no longer being powered for the life of the device.

- **The LTR-553ALS is not fitted on this board.** Settled while the 1.8V rail was still brought up at boot: `0x28` (the BHI260AP, on that same rail) answered the I2C scan while `0x23` never did, cold or with `BOARD_1V8_EN` driven high and 50ms to settle. So the rail was known good and the part is simply absent. Do not expect to repeat that scan now - the rail starts down, and `LTR553_init()` is what raises it.

  **`peri_ltr553.cpp` owns the 1.8V rail**, through `sensor_rail_set()` / `sensor_rail_is_on()`, because it is the only part left on it. `LTR553_init()` raises the rail, gives it 50ms, probes, and puts it back down if nothing answers; the *Power Sensors* switch in the settings goes through the same two calls rather than keeping its own copy of the state, since a mirror initialised to `true` would have disagreed with a rail that now starts off. Ear detect is written against it and starts working the moment one answers, but here it reports "No sensor" and does nothing. Do not go looking for a software cause again - three were investigated and none of them was it.
- **The touch panel is always missing from the boot I2C scan and always works.** The scan runs before `touch.setPins()` releases its reset, and a part in reset does not acknowledge. The scan says as much in its own output, because otherwise it looks exactly like the LTR-553ALS's genuine absence.
...
- **The LTR-553ALS light channel is in the same position** - initialised at boot, `LTR_553ALS_get_channel()` and `LTR_553ALS_get_ps()` never called - except that it is still started, and still appears on the boot screen.
...
- **Signing (commands 33/34/35) accumulates before it signs.** A signature covers the whole message or it is worth nothing, so `CMD_SIGN_DATA` appends into an 8KB buffer that `CMD_SIGN_START` allocates and `CMD_SIGN_FINISH` consumes and frees. The buffer is PSRAM: it is large, held only for one exchange, and internal memory on this board is spoken for. `CMD_APP_START` releases it too, since an app that reconnects part way through a signing exchange is not going to finish the old one.
- **No command answers OK without doing the thing.** This was audited once and found wanting in six places, so it is worth stating as a rule: a command either performs its function, or is refused. Where a command carries several settings and only some can be honoured, the ones that cannot say so in the log rather than being counted as done - `CMD_SET_OTHER_PARAMS` drops only the telemetry modes, and prints when it does.

  The settings behind the tuning commands are real MeshCore virtuals that `PhoneMesh` overrides - `getAirtimeBudgetFactor()`, `calcRxDelay()`, `getExtraAckTransmitCount()`, `isAutoAddEnabled()` - so a value stored is a value used. `CMD_SET_FLOOD_SCOPE_KEY` and `CMD_SET_DEFAULT_FLOOD_SCOPE` agree only to what is already true of an unscoped node (send unscoped, clear the override) and refuse an actual scope key, because agreeing would claim to confine traffic that goes everywhere.

  **Group channels are kept in RAM by MeshCore and nowhere else**, so `CMD_SET_CHANNEL` calls `mesh_net_save_channels()`; without it a channel added from an app was gone by the next restart. `channels_load()` runs after the public channel is added, so a saved channel can replace it.
- **What is still not implemented, and why.** Everything the protocol defines is now handled except one family and four one-offs, and all of them refuse rather than pretend. The family is requests that need a *reply* pushed back later - login (26), status (27), binary (50) and anonymous (57) requests - which need `onContactResponse()` and `onTraceRecv()` wired to `PUSH_CODE_LOGIN_SUCCESS`, `_LOGIN_FAIL`, `_STATUS_RESPONSE` and `_BINARY_RESPONSE`. `BaseChatMesh` has `sendLogin()`, `sendRequest()` and `sendAnonReq()` ready; only the push half is missing. The one-offs are trace path (36), path discovery (52), control data (55) and telemetry (39) - the first two need packet types this node does not compose, and telemetry needs sensor data it does not have.
- **`CMD_EXPORT_CONTACT` for this node builds its own advert, so it needs the position putting in.** `createSelfAdvert(name)` and `createSelfAdvert(name, lat, lon)` are different calls, and the exported packet is what an app submits to put the node on a map - so building it with the name-only overload left it carrying no location however good the fix was. The advert this node broadcasts goes through `advert_send()` and had the position all along; the one handed to the app did not. Any new path that composes a self-advert has to make the same choice.
- **The private key commands (23, 24) answer `RESP_CODE_DISABLED`**, which is the protocol's own way of saying a build will not do that. Handing the identity key out over a link, or letting one be written in, is not something this firmware does: the key *is* the node, and everyone who knows this node knows it by that key.
...
- **A key held at boot is read before `keypad.flush()`, or not at all.** The TCA8418 reports a key that is already down when `matrix()` starts the scanner as an ordinary press in its FIFO, and `keypad_init()` flushes that FIFO before handing the keyboard to LVGL. `keypad_capture_boot_keys()` therefore runs between the two, and is the only chance to see it. Held keys are logged with a count on every boot, since a count of zero while a key is held is the one symptom that would say this does not work on a given keypad.

  `main.cpp` checks for space before `lvgl_init()`, not after: `mesh_companion_link_saved()` decides where the drawing buffer goes, so the veto has to be in place before that question is asked.
- **`link_effective()` is what decides the link, not `link_wanted`.** Two things override the setting without changing it - a key held at boot (`link_held_off`) and the LoRa module being powered down (`link_blocked`) - so that switching either back restores what the user chose. Anything new that suppresses the link belongs there rather than writing to `link_wanted`.
...
- **The GPS task runs while the GPS screen is open, or while the mesh wants a position.** `entry3` resumes it; `exit3` suspends it only when `mesh_net_wants_gps()` is false. The vendor's design was the screen alone, which is reasonable for a battery and was a hole under position sharing: the cached coordinates only ever advanced while somebody was looking at them, so an advert shared whatever was last seen or nothing at all. `gps_follow_policy()` in `mesh_net.cpp` is the one place that decides, and it is called at init, on a policy change, and when a position is set by hand.
