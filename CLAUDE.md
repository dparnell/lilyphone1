...
- **There is no magnetometer on this board, so a compass is not possible.** Settled by asking the BHI260AP itself: it presents 33 virtual sensors and not one of them is a magnetometer (19, 21-25) or Orientation (43). The giveaway is `Game rotation vector` (37) - Bosch's six-axis fusion, the one used precisely when there is no magnetometer, whose yaw is relative to wherever it started and drifts. The only absolute heading available on this device is GPS course over ground, and only while moving.
  ...
  The BHI260AP is a hub rather than a fixed set of sensors, which is why this had to be asked at runtime: the accelerometer and gyroscope are its own, a magnetometer would be an external part on its secondary bus, and `0x28` answers on the main bus either way. `BHI260AP_init()` still attempts `SENSOR_ID_MAG_PASS` and `SENSOR_ID_ORI` and records the answer in `BHI260AP_has_compass()`, and still calls `printSensors()` - so a board that does have one needs only the init call restored to find out.
...
- **The BHI260AP is not started.** Nothing reads it, and its init uploads a firmware image to the sensor over I2C and then streams accelerometer and gyroscope at 100Hz that nobody services. Not calling it also drops SensorLib and the blob it carries - about 115KB of flash. `src/peripherals/peri_gyroscope.cpp` is kept whole, so restoring it is one line in `setup()` plus a row in `boot_screen.cpp`; whoever does will also need to service `bhy.update()`. What the hub does offer, if it is ever wanted: step counting (52), pickup and glance gestures (61, 59), significant motion (55), stationary detect (75) and activity recognition (63).

- **The LTR-553ALS is not fitted on this board.** Settled by the boot I2C scan: `0x28` (the BHI260AP, on the same 1.8V rail) answers while `0x23` never does, cold or with `BOARD_1V8_EN` driven high and 50ms to settle. The rail is fine; the part is simply absent. Ear detect is written against it and starts working the moment one answers, but here it reports "No sensor" and does nothing. Do not go looking for a software cause again - three were investigated and none of them was it.
