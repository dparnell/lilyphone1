...
- **The BHI260AP is a sensor hub, so what it has is a runtime question.** The accelerometer and gyroscope are its own; a magnetometer is an external part on its secondary bus and may not be fitted. Nothing on the main I2C bus can tell you - `0x28` answers either way. `bhy.configure()` refuses a sensor the hub does not present, so `BHI260AP_init()` attempts `SENSOR_ID_MAG_PASS` and `SENSOR_ID_ORI` and records what it got in `BHI260AP_has_compass()`. It also calls `printSensors()` once, which lists every virtual sensor the hub offers.
...
- **The BHI260AP was briefly not started at all**, since nothing read it and its init uploads a firmware image over I2C; it is back for the compass. If it is ever removed again, note that dropping the call also drops SensorLib and its blob - about 115KB of flash.
