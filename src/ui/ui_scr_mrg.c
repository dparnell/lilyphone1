...
static scr_card_t *scr_mgr_find_by_id(int id) // Check if there is no such id in the linked list
{
...
static scr_card_t *scr_mgr_get_top(scr_card_t *head) // Get the first node of the linked list
{
    scr_card_t *p = head;
...
static void scr_mgr_active(scr_card_t *card)  // Set the screen card to active 
{
...
bool scr_mgr_register(int id, scr_lifecycle_t *card_life) // Register a screen card
{
...
bool scr_mgr_switch(int id, bool anim)  // Clear the stack and switch to the screen card with the specified id
{
    ...
    scr_mgr_active(stack_scr); // Set the screen card to active
...
bool scr_mgr_push(int id, bool anim)
{
    ...
    if((scr_stack_top != NULL) && (tgt_card->id == scr_stack_top->id)){ // The current push screen is the same as the top screen
        return false;
    }
