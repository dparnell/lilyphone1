...
**Calendar.** A month at a time, with today boxed, the selected day inverted
and a mark under any day that has something on it; the selected day's events
are listed underneath, earliest first, and tapping the month name comes back to
today. *+* adds an event on the selected day, and tapping an event opens it.

An event has a title, a date, a time or *All day*, notes, a repeat rule and a
reminder. Repeats are once, every day, weekdays, every week, every month or
every year, and a repeating event is one record however many times it happens -
edit it and every occurrence changes. Reminders can be at the time, or 5, 15
or 30 minutes, 1 or 2 hours, or a day before; when one comes due the phone
buzzes and puts the title and time on screen over whatever you were doing,
lock screen included. Each occurrence reminds once, and that is remembered
across a restart. Up to 64 events, kept on the flash.

Times are wall-clock local: an event at 09:00 is at 09:00 in whatever zone the
phone is set to when the day comes round. Reminders wait until the clock has
been set from the network or a GPS fix, since until then the phone does not
...
The home-screen icons for the calculator and the calendar are generated by
`tools/gen_menu_icons.py` (needs pillow) in the same 50x50 format as the
vendor's, so they can be redrawn rather than pixel-edited.
